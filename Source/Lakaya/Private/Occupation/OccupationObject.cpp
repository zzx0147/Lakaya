...
void AOccupationObject::OnInteractionStart(const float& Time, APawn* Caller)
{
	...
	GetWorldTimerManager().SetTimer(InteractionTimerHandle, FTimerDelegate::CreateWeakLambda(this, [this, Caller]
	{
...
void AOccupationObject::OnInteractionFinish(APawn* Caller)
{
	...
	// 만약 성공한 플레이어 팀이 A라면
	if (CallerState->IsSameTeam(ETeam::Anti))
	{
...
void AOccupationObject::SetTeamObject(const ETeam& Team)
{
	ObjectTeam = Team;
}