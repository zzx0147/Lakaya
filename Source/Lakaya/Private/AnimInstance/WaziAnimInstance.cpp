...
void UWaziAnimInstance::NativeBeginPlay()
{
	...
	if(const auto Character = Cast<AArmedCharacter>(TryGetPawnOwner()))
	{
		...
		{
			Ability->OnPerformTimeNotified.AddUObject(this, &UWaziAnimInstance::OnPrimaryAbilityPerformTimeNotified);
		}
		if (const auto Ability = Character->FindAbility<UCoolTimedSummonAbility>(Secondary))
		{
			Ability->OnPerformTimeNotified.AddUObject(this, &UWaziAnimInstance::OnSecondaryAbilityPerformTimeNotified);
		}
	}
}
...
void UWaziAnimInstance::OnPrimaryAbilityPerformTimeNotified(const float& Time)
{
...
void UWaziAnimInstance::OnSecondaryAbilityPerformTimeNotified(const float& Time)
{
