...
#include "UI/OverlayOccupationMinimapWidget.h"

#include "Character/LakayaBaseCharacter.h"
#include "Components/CanvasPanelSlot.h"

void UOverlayOccupationMinimapWidget::NativeConstruct()
{
...
void UOverlayOccupationMinimapWidget::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
	Super::NativeTick(MyGeometry, InDeltaTime);
}

FVector2d UOverlayOccupationMinimapWidget::ConvertWorldToMiniMapCoordinates(const FVector2D& PlayerLocation,
	const FVector2D& MiniMapSize)
{
	return Super::ConvertWorldToMiniMapCoordinates(PlayerLocation, MiniMapSize);
}

UImage* UOverlayOccupationMinimapWidget::CreatePlayerImage(const ETeam& NewTeam, const bool bMyPlayer)
{
...
void UOverlayOccupationMinimapWidget::UpdatePlayerPosition(const ETeam& Team)
{
...
void UOverlayOccupationMinimapWidget::SetEnemyImage() const
{
...
void UOverlayOccupationMinimapWidget::UpdatePlayerPosition(const ETeam& NewTeam,
...
#pragma endregion
	...
	const FVector2D NewPlayerPosition = const_cast<UOverlayOccupationMinimapWidget*>(this)->ConvertWorldToMiniMapCoordinates(PlayerPosition, MinimapSize);
...
void UOverlayOccupationMinimapWidget::HidePlayerPosition(const ETeam& NewTeam,
	const TWeakObjectPtr<ALakayaBasePlayerState> NewPlayerState)
{
	if (const TWeakObjectPtr<ALakayaBasePlayerState> WeakNewPlayerState = NewPlayerState; !PlayersByMinimap[NewTeam].Contains(WeakNewPlayerState))
	{
...
void UOverlayOccupationMinimapWidget::UpdateMinimapImagePositionAndRotation(const ALakayaBasePlayerState& NewPlayerState,
